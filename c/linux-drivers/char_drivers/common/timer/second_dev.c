/******************************************    *******************************
      > File Name: second_dev.c
      > Author: Ywl
      > Descripsion:
      > Created Time:     2017年11月27日 星期一 16时16分15秒
      > Modify Time:
 *********************************    ***************************************/
...
static int second_open(struct inode *inode, struct file *filp)
{
	...
	atomic_set(&second_devp->counter, 0);				/*初始化秒计数器为0*/
	return 0;
}
...
static ssize_t second_read(struct file *filp, char __user *buf, size_t count, loff_t *fpos)
{
	...
	if(put_user(counter, (int *)buf))		/*复制到用户空间*/
		return -EFAULT;
	else
...
static void second_dev_setup(struct second_dev *dev, int index)
{
...
static int __init second_init(void)
{
	...
	second_dev_setup(second_devp, 0);
	return ret;
fail_malloc:
